...
def unify_timestamps_vtt(text):
    pattern_1_digit = re.compile(r'(\d{1}:\d{2}:\d{2}\.\d)(?!\d)')
    pattern_2_digits = re.compile(r'(\d{1}:\d{2}:\d{2}\.\d{2})(?!\d)')
...
def convert_docx_to_srttxt(docx_files):
    ...
    for docx_file in docx_files:
        try:
            ...
            print(f"Initial content read from file: {content[:100]}")  # Show only the first 200 characters for brevity
